...
export class AuthenticateWithGoogleUsecase {
    ...
    async execute({ idToken, role }: authenticateWithGoogleInput): Promise<authenticateWithGoogleOutput> {
      ...
      const [accessToken, refreshToken] = await Promise.all([
        this.jwt.generateAccessToken({ userId: user.getId(), role: user.getRole() }),
